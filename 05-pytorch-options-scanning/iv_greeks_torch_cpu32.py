...
- torch.compile fuses the pricer; each Newton step calls the compiled kernel
- _BUCKET padding gives jit a fixed shape so it compiles once per bucket boundary
- fp32 variant of iv_greeks_torch_cpu.py

Single-function fusion (vs the earlier split solve_iv/greeks API):
- inputs convert to tensors once instead of twice
- the Newton loop's final-iteration price & vega are reused as the Greek
  base/vega: 95 pricer evals instead of 98. base/vega sit at sigma_29 while
  the bump evals sit at sigma_30; at convergence those agree, so iv/delta/rho
  match the split engines bit-for-bit and price/gamma/vega/theta match to
  ~1e-8 at p99.9.
"""
import math
...
def _in(a):
    return torch.as_tensor(np.asarray(a, dtype=np.float32), device=_DEV)
...
def _bs2002_call(fs, x, t, r, b, v):
    ...
    beta_m1 = torch.where(torch.abs(beta - 1.0) < 1e-6, 1e-6, beta - 1.0)
    b_inf = (beta / beta_m1) * x
    rmb = torch.where(torch.abs(r - b) < 1e-6, 1e-6, r - b)
...
# whose CPU codegen never finishes (>23 min observed). PyTorch has no
# clean fori_loop, so we fuse the bounded pricer graph and keep the loop eager:
# each iteration calls the compiled kernel. Same per-eval fusion, no explosion.
# The eager loop is also what makes the base/vega reuse below free: they're
# just Python references to the last iteration's tensors.
_price = torch.compile(_price, fullgraph=True, mode="default")


def _solve_greeks(price, S, K, T, r, q, ic):
    """Newton IV solve, then Greeks -- one pass.
...
def solve_iv_greeks(price, S, K, T, r, q, is_call):
    n = len(price)
    ic = torch.as_tensor(np.asarray(is_call), device=_DEV)
    price_t, S_t, K_t, T_t = [_in(a) for a in (price, S, K, T)]
    r_t = torch.full((n,), float(r), dtype=torch.float32, device=_DEV)
    q_t = torch.full((n,), float(q), dtype=torch.float32, device=_DEV)
    price_t, S_t, K_t, T_t, r_t, q_t, ic = _padded(
        [price_t, S_t, K_t, T_t, r_t, q_t, ic],
        [1.0, 100.0, 100.0, 0.5, float(r), float(q), True], n)
    iv_t, price_o, delta_t, gamma_t, vega_t, rho_t, theta_t = \
        _solve_greeks(price_t, S_t, K_t, T_t, r_t, q_t, ic)
    slc = lambda t: t.numpy()[:n]
    return slc(iv_t), {"price": slc(price_o), "delta": slc(delta_t), "gamma": slc(gamma_t),
                       "vega": slc(vega_t), "rho": slc(rho_t), "theta": slc(theta_t)}
