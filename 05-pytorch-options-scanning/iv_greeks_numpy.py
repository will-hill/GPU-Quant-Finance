...
xp.seterr(all="ignore")        # BS2002 evaluates both call/put branches; unused one overflows
...
def _out(a):
    return a                   # already a NumPy array on the host
...
def _bs2002_call(fs, x, t, r, b, v):
    ...
    beta_m1 = xp.where(xp.abs(beta - 1.0) < 1e-12, 1e-12, beta - 1.0)
    b_inf = (beta / beta_m1) * x
    rmb = xp.where(xp.abs(r - b) < 1e-12, 1e-12, r - b)
...
def solve_iv_greeks(price, S, K, T, r, q, is_call, n_iter=30,
    ...
    r = xp.full(price.shape, float(r))
    q = xp.full(price.shape, float(q))
